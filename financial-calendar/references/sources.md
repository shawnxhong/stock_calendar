...
- BLS 可能由 Akamai 对自动请求返回 403。按官方日历订阅用途发送标准
  calendar/browser 请求头，但不规避访问控制；若仍失败则记为 `advisory`，继续使用
  FRED 日期与静态时点，并用软提示说明本次仅失去 BLS 时点交叉验证。此项不把
  health 降为 `degraded`，因为主要事件数据没有缺失。
